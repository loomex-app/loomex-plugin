...
- Call `loomex_workflow_create` exactly once for each user request. Once it returns an `agentTask`, never call `loomex_workflow_create` again for that request; all later attempts use `loomex_workflow_create_respond`.
...
- Follow the current `agentTask.sessionDirective` exactly: `spawn` creates one new Codex sub-agent and submits its actual id with action `spawned`; `resume` resumes only `sessionDirective.sessionId` and submits that same id with action `resumed`. Never spawn a replacement for `resume` and never submit a repair with action `spawned`.
...
- Submit the sub-agent result using `loomex_workflow_create_respond` with `builderSession.id` as `sessionId`—this is not the sub-agent id—and a fresh idempotency key. Include the actual sub-agent session object with `id`, `host`, `action`, `provider`, and `model`.
- If the response contains another `agentTask` and `validationErrors`, use the new repair prompt with the same sub-agent session, then submit the repaired JSON through `loomex_workflow_create_respond`. Never call `loomex_workflow_create` again and never spawn a replacement for a repair attempt.

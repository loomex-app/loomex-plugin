...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct RunnerJobCancellationDirective {
...
impl RunnerJobCancellationDirective {
    pub fn from_runner_job(job: &Value) -> CoreResult<Self> {
        if job.get("kind").and_then(Value::as_str) != Some("agent.execute.v2")
            || job.get("status").and_then(Value::as_str) != Some("canceling")
        {
            return Err(CoreError::new(
                "RUNNER_JOB_CANCELLATION_INVALID",
                "runner cancellation must belong to a canceling agent.execute.v2 job",
            ));
        }
        let job_id = required_uuid_string(job, "/id", "runner cancellation job id")?;
        let session_id =
            required_nonempty_string(job, "/sessionId", "runner cancellation session id")?;
        let job_lease_version =
            required_positive_u64(job, "/leaseVersion", "runner cancellation lease version")?;
        let job_binding_generation = required_positive_u64(
            job,
            "/bindingGeneration",
            "runner cancellation binding generation",
        )?;
        let cancellation = job.pointer("/metadata/cancellation").ok_or_else(|| {
            CoreError::new(
                "RUNNER_JOB_CANCELLATION_INVALID",
                "runner cancellation metadata is missing",
            )
        })?;
        let directive = Self {
            cancellation_id: required_uuid_string(
                cancellation,
                "/cancellationId",
                "runner cancellation id",
            )?,
            job_id: required_uuid_string(
                cancellation,
                "/jobId",
                "runner cancellation metadata job id",
            )?,
            session_id,
            process_attempt_id: required_uuid_string(
                cancellation,
                "/processAttemptId",
                "runner cancellation process attempt id",
            )?,
            lease_version: required_positive_u64(
                cancellation,
                "/leaseVersion",
                "runner cancellation metadata lease version",
            )?,
            binding_generation: required_positive_u64(
                cancellation,
                "/bindingGeneration",
                "runner cancellation metadata binding generation",
            )?,
            requested_at: required_nonempty_string(
                cancellation,
                "/requestedAt",
                "runner cancellation requested timestamp",
            )?,
            state: required_nonempty_string(cancellation, "/state", "runner cancellation state")?,
        };
        if directive.job_id != job_id
            || directive.lease_version != job_lease_version
            || directive.binding_generation != job_binding_generation
            || !matches!(directive.state.as_str(), "requested" | "acknowledged")
        {
            return Err(CoreError::new(
                "RUNNER_JOB_CANCELLATION_INVALID",
                "runner cancellation metadata does not match its authoritative job",
            ));
        }
        Ok(directive)
    }

    pub fn acknowledgement_idempotency_key(&self) -> String {
        format!("backend-cancel:{}", self.cancellation_id)
    }
}

fn required_uuid_string(value: &Value, pointer: &str, field: &str) -> CoreResult<String> {
    let raw = required_nonempty_string(value, pointer, field)?;
...
fn required_nonempty_string(value: &Value, pointer: &str, field: &str) -> CoreResult<String> {
    let raw = value
        .pointer(pointer)
        ...
        .ok_or_else(|| {
            CoreError::new(
                "RUNNER_JOB_CANCELLATION_INVALID",
                format!("{field} is missing or invalid"),
            )
        })?;
    Ok(raw.to_string())
}

fn required_positive_u64(value: &Value, pointer: &str, field: &str) -> CoreResult<u64> {
    value
        .pointer(pointer)
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct RunnerJobCancellationAckResponse {
    pub cancellation_id: String,
    pub state: String,
    pub replayed: bool,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginAgentCancellationRequest {
    pub expected_process_attempt_id: String,
    pub expected_runner_job_id: Option<String>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginAgentCancellationReceipt {
    pub request_id: String,
    pub agent_execution_id: String,
    pub sequence: u64,
    pub process_attempt_id: String,
    pub cancellation: PluginAgentCancellationResource,
    pub job: Option<PluginAgentCancellationJob>,
    pub local_cancellation_authorized: bool,
    pub replayed: bool,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginAgentCancellationResource {
    pub id: String,
    pub state: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginAgentCancellationJob {
    pub id: String,
    pub status: String,
    pub lease_version: u64,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginAgentSuccessorRequest {
    pub expected_process_attempt_id: String,
    pub expected_binding_generation: u64,
    pub expected_checkpoint_id: String,
    pub reason: String,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginAgentSuccessorReceipt {
    pub request_id: String,
    pub agent_execution_id: String,
    pub sequence: u64,
    pub predecessor: PluginAgentSuccessorPredecessor,
    pub successor: PluginAgentSuccessorProcess,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginAgentSuccessorPredecessor {
    pub process_attempt_id: String,
    pub state: String,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginAgentSuccessorProcess {
    pub process_attempt_id: String,
...
pub trait ManagementApiClient {
    ...
    fn request_plugin_agent_cancellation(
        &mut self,
        _credential: &ManagementCredential,
        _request_id: &str,
        _request: &PluginAgentCancellationRequest,
        _idempotency_key: &str,
    ) -> CoreResult<PluginAgentCancellationReceipt> {
        Err(CoreError::new(
            "PLUGIN_AGENT_CANCELLATION_UNSUPPORTED",
            "management client does not implement plugin-agent cancellation",
        ))
    }
    fn request_plugin_agent_successor(
        &mut self,
        _credential: &ManagementCredential,
        _request_id: &str,
        _request: &PluginAgentSuccessorRequest,
        _idempotency_key: &str,
    ) -> CoreResult<PluginAgentSuccessorReceipt> {
        Err(CoreError::new(
            "PLUGIN_AGENT_SUCCESSOR_UNSUPPORTED",
            "management client does not implement plugin-agent successor authorization",
        ))
    }
    ...
    fn list_runner_job_cancellation_directives(
        ...
        credential: &ManagementCredential,
        session_id: &str,
    ) -> CoreResult<Vec<RunnerJobCancellationDirective>> {
        self.list_runner_job_cancellations(credential, session_id)?
            .iter()
            .map(RunnerJobCancellationDirective::from_runner_job)
            .collect()
    }
    fn acknowledge_runner_job_cancellation(
        ...
        _idempotency_key: &str,
    ) -> CoreResult<RunnerJobCancellationAckResponse> {
        Err(CoreError::new(
            "RUNNER_JOB_CANCELLATION_ACK_UNSUPPORTED",
            "management client does not implement runner cancellation acknowledgement",
        ))
    }
    fn lease_runner_job(
        &mut self,
    ...
    fn defer_runner_job_idempotent(
        ...
        Err(CoreError::new(
            "RUNNER_JOB_DEFER_UNSUPPORTED",
            "management client does not implement runner job defer",
        ))
    }
...
impl ManagementApiClient for HttpManagementApiClient {
    ...
    fn request_plugin_agent_cancellation(
        ...
        credential: &ManagementCredential,
        request_id: &str,
        request: &PluginAgentCancellationRequest,
        idempotency_key: &str,
    ) -> CoreResult<PluginAgentCancellationReceipt> {
        if credential.kind != CredentialKind::User {
            return Err(CoreError::new(
                "AGENT_CANCELLATION_AUTHORIZATION_REQUIRED",
                "plugin-agent cancellation requires an authenticated user credential",
            ));
        }
        loomex_protocol::agent_runtime_v2::validate_idempotency_key(idempotency_key).map_err(
            |_| {
                CoreError::new(
                    "IDEMPOTENCY_KEY_INVALID",
                    "Idempotency-Key must use the protocol-safe grammar and not exceed 160 bytes",
                )
            },
        )?;
        let response = self
            .post_with_auth(
                &format!(
                    "/workflow-runtime/plugin-agent-requests/{}/cancellations/",
                    encode_path(request_id)
                ),
                credential,
            )
            ...
            .send()
            .map_err(|err| CoreError::new("MANAGEMENT_HTTP_FAILED", err.to_string()))?;
        let envelope: ClientEnvelope<PluginAgentCancellationReceipt> =
            parse_json_response(response)?;
        Ok(envelope.data)
    }
    ...
    fn request_plugin_agent_successor(
        ...
        credential: &ManagementCredential,
        request_id: &str,
        request: &PluginAgentSuccessorRequest,
        idempotency_key: &str,
    ) -> CoreResult<PluginAgentSuccessorReceipt> {
        if credential.kind != CredentialKind::User {
            return Err(CoreError::new(
                "AGENT_SUCCESSOR_AUTHORIZATION_REQUIRED",
                "plugin-agent successor authorization requires an authenticated user credential",
            ));
        }
        loomex_protocol::agent_runtime_v2::validate_idempotency_key(idempotency_key).map_err(
            |_| {
        ...
        let response = self
            .post_with_auth(
                &format!(
                    "/workflow-runtime/plugin-agent-requests/{}/successors/",
                    encode_path(request_id)
                ),
                credential,
            )
            ...
            .send()
            .map_err(|err| CoreError::new("MANAGEMENT_HTTP_FAILED", err.to_string()))?;
        let envelope: ClientEnvelope<PluginAgentSuccessorReceipt> = parse_json_response(response)?;
        if envelope.data.sequence == 0 {
            return Err(CoreError::new(
                "AGENT_SUCCESSOR_RESPONSE_INVALID",
                "Backend successor response sequence must be positive",
            ));
        }
        Ok(envelope.data)
    }
    ...
    fn acknowledge_runner_job_cancellation(
        ...
        loomex_protocol::agent_runtime_v2::validate_idempotency_key(idempotency_key).map_err(
            |_| {
                CoreError::new(
                    "IDEMPOTENCY_KEY_INVALID",
                    "cancellation acknowledgement idempotencyKey must use the protocol-safe grammar",
                )
            },
        )?;
        let response = self
            .post_with_auth(
                &format!(
                    "/runner-control/runner/v1/jobs/{}/cancellation-ack/",
                    encode_path(job_id)
                ),
                credential,
            )
            ...
            .send()
            .map_err(|err| CoreError::new("MANAGEMENT_HTTP_FAILED", err.to_string()))?;
        let envelope: ClientEnvelope<RunnerJobCancellationAckResponse> =
            parse_json_response(response)?;
        if envelope.data.cancellation_id != cancellation_id
            || !matches!(envelope.data.state.as_str(), "acknowledged" | "completed")
        {
            return Err(CoreError::new(
                "RUNNER_JOB_CANCELLATION_ACK_INVALID",
                "runner cancellation acknowledgement did not match the requested cancellation",
            ));
        }
        Ok(envelope.data)
    }
    ...
    fn defer_runner_job_idempotent(
        ...
        let response = self
            .post_with_auth(
                &format!(
                    "/runner-control/runner/v1/jobs/{}/defer/",
                    encode_path(job_id)
                ),
                credential,
            )
...
mod tests {
    ...
    #[test]
    fn runner_cancellation_directive_requires_exact_authoritative_job_metadata() {
        let job_id = "11111111-1111-4111-8111-111111111111";
        let cancellation_id = "22222222-2222-4222-8222-222222222222";
        let process_id = "33333333-3333-4333-8333-333333333333";
        let job = json!({
            "id": job_id,
        ...
        let directive = RunnerJobCancellationDirective::from_runner_job(&job).unwrap();
        assert_eq!(cancellation_id, directive.cancellation_id);
        assert_eq!(
            "backend-cancel:22222222-2222-4222-8222-222222222222",
            directive.acknowledgement_idempotency_key()
        );

        for pointer in [
            "/metadata/cancellation/jobId",
            "/metadata/cancellation/processAttemptId",
            "/metadata/cancellation/cancellationId",
        ] {
            let mut malformed = job.clone();
            *malformed.pointer_mut(pointer).unwrap() = Value::String("not-a-uuid".to_string());
            assert_eq!(
                "RUNNER_JOB_CANCELLATION_INVALID",
                RunnerJobCancellationDirective::from_runner_job(&malformed)
                    .unwrap_err()
                    .code
            );
        }
        let mut stale = job;
        stale["metadata"]["cancellation"]["leaseVersion"] = json!(3);
        assert_eq!(
            "RUNNER_JOB_CANCELLATION_INVALID",
            RunnerJobCancellationDirective::from_runner_job(&stale)
                .unwrap_err()
                .code
        );
    }
    ...
    #[test]
    fn runner_cancellation_ack_uses_runner_auth_and_frozen_wire_contract() {
        let response = r#"{"data":{"cancellationId":"22222222-2222-4222-8222-222222222222","state":"acknowledged","replayed":false}}"#;
        let (base_url, receiver, server) = serve_one_http_response(response);
        let mut client = HttpManagementApiClient::new(base_url, None).unwrap();
        let credential = test_credential("runner-secret");
        ...
        let receipt = client
            ...
            .unwrap();
        let request = captured_request(receiver, server);

        assert_eq!("acknowledged", receipt.state);
        assert!(request.starts_with("POST "), "{request}");
        assert!(
            request.contains(
                "/runner-control/runner/v1/jobs/11111111-1111-4111-8111-111111111111/cancellation-ack/"
            ),
            "{request}"
        );
        assert!(request
            .to_ascii_lowercase()
            .contains("authorization: bearer runner-secret"));
        assert!(request
            .to_ascii_lowercase()
            .contains("idempotency-key: backend-cancel:22222222-2222-4222-8222-222222222222"));
        let body: Value = serde_json::from_str(request.split("\r\n\r\n").nth(1).unwrap()).unwrap();
        assert_eq!(
            json!({
                "sessionId": "44444444-4444-4444-8444-444444444444",
                "leaseVersion": 4,
                "cancellationId": "22222222-2222-4222-8222-222222222222",
                "status": "accepted",
                "idempotencyKey": "backend-cancel:22222222-2222-4222-8222-222222222222"
            }),
            body
        );
    }

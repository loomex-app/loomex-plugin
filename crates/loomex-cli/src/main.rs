...
fn run(args: Vec<String>) -> Result<String, String> {
    ...
    // compensation cannot race a config, identity, or service change.
...
fn run_login_with<C: ManagementApiClient, S: CredentialStore>(
    ...
    if user_token {
        ...
        // the org-scoped runner credential used by the local execution service.
...
const RUNNER_REAUTH_GUIDANCE: &str = "stored runner credential predates runner-control v1 or lacks runner scopes; sign out and authenticate again, then restart Runner authentication";
...
fn plugin_stop_service_and_invalidate_local_control(
    ...
    if !active && plugin_local_control_ping_once().is_ok() {
        return Err(
            "RUNNER_SERVICE_UNMANAGED_ACTIVE: a healthy local daemon is not owned by the configured service; stop it before changing identity"
                .to_string(),
        );
    }
...
fn plugin_bootstrap_runner_auth(options: &GlobalOptions) -> Result<Value, String> {
    ...
    let mut client =
        HttpManagementApiClient::new(&resolved.server_url, resolved.host_header.clone())
            .map_err(format_core_error)?;
    let exchange = client
        .bootstrap_runner_with_workspace_token(&user_credential.access_token, &organization_id)
        .map_err(format_core_error)?;
...
fn run_runner_start_with_config_path(
    ...
    let resolved = config
        .resolve(options.config_overrides(), |key| env::var(key).ok())
        .map_err(format_core_error)?;
    let Some(runner_id) = resolved.runner_id.as_deref() else {
        return parsed_stub("runner start", &[], options);
    };
    let guard = acquire_runner_runtime_guard(config_path, runner_id, "loomex-cli")
        .map_err(format_core_error)?;
...
fn run_runner_stop_with_config_path(
    ...
    let guard_path = runner_runtime_guard_path(config_path, runner_id);
    release_runner_runtime_guard_for_surface(&guard_path, runner_id, "loomex-cli")
        .map_err(format_core_error)?;
    if options.json {
        return Ok(json!({
...
fn run_runner_service_run(args: &[String], options: &GlobalOptions) -> Result<String, String> {
    ...
    run_runner_service_run_parsed(service_options, options, &resolved, &store, &mut client)
}
...
fn run_runner_service_run_parsed<C: ManagementApiClient>(
    ...
    let runner_scope_id = resolved.runner_id.as_deref().unwrap_or(&resolved.profile);
    let guard = acquire_runner_runtime_guard(
        &service_options.config_path,
        runner_scope_id,
        "loomex-service",
    )
    ...
    if service_options.once {
        let start = run_runner_control_service_once(client, &credential, resolved, &mut recovery)?;
        append_runner_service_log(
            &log_path,
            &credential,
            "info",
        ...
        if options.json {
            return Ok(json!({
                ...
                "fallback": false,
                "profile": resolved.profile,
                "runnerId": runner_scope_id,
                "guardPath": guard.path()
            })
            .to_string());
        }
        return Ok(format!(
            "runner service ready\nrunner: {runner_scope_id}\nguard: {}",
            guard.path().display()
        ));
    }
    run_runner_control_service_loop(client, &credential, resolved, &mut recovery, &log_path)?;
    drop(guard);
    Ok(String::new())
}
...
fn run_runner_control_service_once<C: ManagementApiClient>(
    ...
    let session = create_runner_control_session(client, credential, resolved)?;
...
fn run_runner_control_session<C: ManagementApiClient>(
    ...
    loop {
        ...
        if idle_ticks.is_multiple_of(10) {
            client
                .heartbeat_runner_session(credential, &session_id, runner_control_manifest(&scoped))
                .map_err(format_core_error)?;
        }
...
fn create_runner_control_session<C: ManagementApiClient>(
    ...
    client
        .create_runner_session(credential, runner_control_manifest(resolved), "long_poll")
        .map_err(format_core_error)
}

fn runner_control_manifest(_resolved: &loomex_core::ResolvedCliSettings) -> Value {
    json!({
        "surface": "plugin",
...
fn execute_cancellable_runner_job<C: ManagementApiClient>(
    ...
    loop {
        ...
        if now_epoch_ms >= next_session_heartbeat_epoch_ms {
            client
                .heartbeat_runner_session(credential, session_id, runner_control_manifest(resolved))
                .map_err(format_core_error)?;
...
fn authorize_runner_path(
    ...
    if !requested.starts_with(&root) {
        return Err(
            "RUNNER_JOB_PATH_OUTSIDE_EXECUTION_ROOT: requested path escapes execution root"
                .to_string(),
        );
    }
    if let Some(resolved_path) = resolved_string.as_deref() {
        if !Path::new(resolved_path).starts_with(&root) {
            return Err(
                "RUNNER_JOB_PATH_OUTSIDE_EXECUTION_ROOT: resolved path escapes execution root"
                    .to_string(),
            );
        }
    }
    let _ = (resolved, session_id, capability, requested_string);
    Ok(())
}
...
fn build_support_bundle(log_limit: usize) -> Result<Value, String> {
    ...
    let mut fake_client =
        HttpManagementApiClient::new(&resolved.server_url, resolved.host_header.clone())
            .map_err(format_core_error)?;
    let status = build_runner_status_report(&resolved, None, &mut fake_client, &log_entries);
...
fn explain_policy(args: &[String]) -> Result<Value, String> {
    ...
    let execution_root =
        loomex_core::ExecutionRoot::new(&workspace_path).map_err(format_core_error)?;
...
fn build_doctor_checks<C: ManagementApiClient>(
    ...
    let Some(credential) = credential else {
        ...
        checks.push(workspace_doctor_check(workspace_override));
...
fn runner_control_transport_doctor_check<C: ManagementApiClient>(
    ...
    let Some(runner_id) = resolved.runner_id.as_deref() else {
        return DoctorCheck::warn(
            "runnerControl",
            "runner-control check skipped until the organization-scoped runner is authenticated",
        );
    };
    ...
    DoctorCheck::ok(
        "runnerControl",
        format!(
        "organization-scoped runner-control long-poll transport is ready for runner {runner_id}"
        ),
    )
}
...
fn wizard_start_output(json_mode: bool) -> String {
    if json_mode {
        return json!({
            ...
            "nextCommands": [
                "loomex login",
                "loomex workflow run WORKFLOW_ID --workspace /srv/my-app --input @input.json"
            ]
        })
        .to_string();
    }
    [
        ...
        "Next: loomex login -> loomex workflow run WORKFLOW_ID --workspace /srv/my-app --input @input.json"
            .to_string(),
    ]
    .join("\n")
}
...
usage:
  ...
  loomex support bundle|diagnostic-request

...
pub fn definitions() -> Vec<ToolDefinition> {
    vec![
        ...
        tool(
            ...
            "Inspect local agent runtimes",
            "Inspect the safe, redacted capability snapshot for the local Codex, Claude, and agy agent runtimes.",
        ...
        tool(
            ...
            "Execute local agent task",
            "Accept a durable v2 agent task by request ID. The runner loads the trusted task; arbitrary prompts, commands, paths, models, providers, arguments, environment values, and credentials are not accepted.",
            "agent.execute",
            agent_task_operation_input_schema(),
            mutating(false, true, true),
        ),
        tool(
            "loomex_agent_task_resume",
            "Resume local agent task",
            "Request a Backend-authorized successor for a durable v2 agent task using a fresh operation idempotency key. The receipt decides exact-session resume or a fresh retry; this tool never spawns locally.",
            "agent.resume",
            agent_control_operation_input_schema(),
            mutating(false, true, true),
        ),
        tool(
            "loomex_agent_task_cancel",
            "Cancel local agent task",
            "Request Backend-authorized cancellation control for a durable v2 agent execution. Backend-owned work is never signaled locally; the receipt may be canceling, completed, indeterminate, or an immediately completed deferred job.",
            "agent.cancel",
            agent_control_operation_input_schema(),
            mutating(true, true, true),
        ),
        tool(
            ...
            "Checkpoint local agent task",
            "Request a durable, redacted session checkpoint for a v2 local agent execution.",
            "agent.checkpoint",
            agent_task_operation_input_schema(),
            mutating(false, true, true),
        ),
    ...
    definitions()
        ...
        .and_then(|tool| {
            let method = match tool.name {
                ...
                "loomex_agent_runtime_status" => "agent.runtime.status",
                "loomex_agent_task_execute" => "agent.execute",
                "loomex_agent_task_resume" => "agent.resume",
                "loomex_agent_task_cancel" => "agent.cancel",
            ...
            let deadline = if matches!(tool.name, "loomex_setup_apply" | "loomex_setup_rollback") {
                DeadlineKind::Setup
            ...
            Some(ToolRoute { method, deadline })
        })
}
...
fn validate_value(schema: &Value, value: &Value, path: &str) -> Result<(), String> {
    ...
    if let Some(text) = value.as_str() {
        ...
        if let Some(max) = schema
            .get("x-loomexCliIdentifierMaxBytes")
            .and_then(Value::as_u64)
        {
            if !is_safe_cli_identifier(text, max as usize) {
                return Err(format!("{path} is not a safe CLI identifier"));
            }
        }
        if let Some(max) = schema
            .get("x-loomexIdempotencyKeyMaxBytes")
            .and_then(Value::as_u64)
        {
            if !is_safe_cli_identifier(text, max as usize) {
                return Err(format!(
                    "{path} must use the protocol-safe idempotency-key grammar"
                ));
            }
        }
        if schema.get("x-loomexRfc3339Timestamp") == Some(&Value::Bool(true))
            && !is_safe_agent_control_timestamp(text)
        {
            return Err(format!("{path} must be a safe RFC 3339 timestamp"));
        }
        if let Some(max) = schema
            .get("x-loomexControlIdentifierMaxBytes")
            .and_then(Value::as_u64)
        {
            if !is_safe_agent_control_identifier(text, max as usize) {
                return Err(format!("{path} is not a safe agent control identifier"));
            }
        }
    }
    if value.is_number() {
...
fn agent_task_operation_input_schema() -> Value {
    obj(
        &[
            ("requestId", agent_control_identifier()),
...
fn agent_control_operation_input_schema() -> Value {
    obj(
        &[
            ("requestId", agent_control_identifier()),
            ("operationIdempotencyKey", agent_idempotency_key()),
        ],
        &["requestId", "operationIdempotencyKey"],
    )
}
...
fn output_error_schema() -> Value {
    strict_object(
        &[
            ...
            ("retryable", boolean()),
            ("remediation", array_of(agent_remediation_action_schema())),
        ],
...
fn output_data_schema(tool_name: &str) -> Value {
    match tool_name {
        ...
        "loomex_agent_runtime_status" => agent_runtime_status_schema(),
        "loomex_agent_task_execute" | "loomex_agent_task_checkpoint" => {
            agent_operation_receipt_schema()
        }
        "loomex_agent_task_resume" => agent_successor_control_schema(),
        "loomex_agent_task_cancel" => agent_cancellation_control_schema(),
        ...
        // Fail closed if a future advertised tool is not wired to an output
        // contract. The protocol layer will also refuse to route an incomplete
        // registry entry with a safe internal JSON-RPC error.
        _ => strict_object(&[], &["__invalid_tool_registry"]),
    }
}

fn agent_runtime_status_schema() -> Value {
    strict_object(
        &[
            ("schema", json!({"const":"loomex.agent-capabilities.v2"})),
            ("observedAt", timestamp_string()),
            ("ttlSeconds", positive_integer()),
            ("runtimes", array_of(agent_runtime_capability_schema())),
        ],
        &["schema", "observedAt", "ttlSeconds", "runtimes"],
    )
}

fn agent_runtime_capability_schema() -> Value {
    strict_object(
        &[
            ("provider", agent_provider_schema()),
            ("executor", agent_executor_schema()),
            ("installed", boolean()),
            (
                "authentication",
                enum_string(&[
            ...
            ("models", array_of(agent_model_capability_schema())),
            ("features", agent_runtime_features_schema()),
        ],
        &[
            "provider",
            "executor",
...
fn agent_model_capability_schema() -> Value {
    strict_object(
        &[
            ("modelKey", agent_model_identifier()),
            ("providerModelId", agent_model_identifier()),
            (
                "availability",
                enum_string(&["available", "unavailable", "unknown"]),
            ),
        ],
        &["modelKey", "providerModelId", "availability"],
    )
}

fn agent_runtime_features_schema() -> Value {
    strict_object(
        &[
...
fn agent_operation_receipt_schema() -> Value {
    let without_model = [
        ("requestId", agent_control_identifier()),
        ("idempotencyKey", agent_idempotency_key()),
        ("executionId", agent_control_identifier()),
        ("state", agent_execution_state_schema()),
        ("accepted", boolean()),
        ("sequence", nonnegative_integer()),
        ("provider", agent_provider_schema()),
        ("executor", agent_executor_schema()),
        ("sessionId", agent_control_identifier()),
        ("error", agent_public_error_schema()),
    ];
    let mut with_model = without_model.to_vec();
    with_model.extend([
        ("modelKey", agent_model_identifier()),
        ("providerModelId", agent_model_identifier()),
    ]);
    ...
    let mut model_required = common_required.to_vec();
    model_required.extend(["modelKey", "providerModelId"]);
    json!({
        "oneOf": [
            strict_object(&without_model, &common_required),
            strict_object(&with_model, &model_required),
        ]
    })
}

fn agent_successor_control_schema() -> Value {
    strict_object(
        &[
            (
                "schemaVersion",
                json!({"const":"loomex.agent-successor-control/v1"}),
            ),
            ("controlState", json!({"const":"queued"})),
            ("requestId", agent_control_identifier()),
            ("agentExecutionId", agent_control_identifier()),
            ("sequence", positive_integer()),
            (
                "predecessor",
                strict_object(
                    &[
                        ("processAttemptId", agent_control_identifier()),
                        ("state", enum_string(&["blocked", "indeterminate"])),
                    ],
                    &["processAttemptId", "state"],
                ),
            ),
            (
                "successor",
                strict_object(
                    &[
                        ...
                        ("jobId", agent_control_identifier()),
                        ("jobStatus", json!({"const":"queued"})),
                    ],
                    &[
                        "processAttemptId",
                        "attemptNumber",
            ...
            ("authorizationId", agent_control_identifier()),
            ("authorizedAt", agent_control_timestamp()),
            ("replayed", boolean()),
        ],
        &[
            "schemaVersion",
...
fn agent_cancellation_control_schema() -> Value {
    let common = [
        (
            "schemaVersion",
            json!({"const":"loomex.agent-cancellation-control/v1"}),
        ),
        ("requestId", agent_control_identifier()),
        ("agentExecutionId", agent_control_identifier()),
        ("sequence", nonnegative_integer()),
        ("processAttemptId", agent_control_identifier()),
        ("localCancellationAuthorized", json!({"const":false})),
    ];
    let cancellation_pending = strict_object(
        &[
            ("id", agent_control_identifier()),
            ("state", enum_string(&["requested", "acknowledged"])),
            ("deliveryRoute", json!({"const":"runner_job"})),
            ("requestedAt", agent_control_timestamp()),
        ],
        &["id", "state", "deliveryRoute", "requestedAt"],
    );
    let cancellation_completed = strict_object(
        &[
            ("id", agent_control_identifier()),
            ("state", json!({"const":"completed"})),
            ("deliveryRoute", json!({"const":"runner_job"})),
            ("requestedAt", agent_control_timestamp()),
        ],
        &["id", "state", "deliveryRoute", "requestedAt"],
    );
    let canceling_job = strict_object(
        &[
            ("id", agent_control_identifier()),
            ("status", json!({"const":"canceling"})),
            ("leaseVersion", positive_integer()),
        ],
        &["id", "status", "leaseVersion"],
    );
    let canceled_job = strict_object(
        &[
            ("id", agent_control_identifier()),
            ("status", json!({"const":"canceled"})),
            ("leaseVersion", positive_integer()),
        ],
        &["id", "status", "leaseVersion"],
    );
    let deferred_job = strict_object(
        &[
            ("id", agent_control_identifier()),
            ("status", json!({"const":"deferred"})),
            ("leaseVersion", positive_integer()),
        ],
        &["id", "status", "leaseVersion"],
    );
    let mut canceling = common.to_vec();
    canceling.extend([
        ("controlState", json!({"const":"canceling"})),
        ("cancellation", cancellation_pending),
        ("job", canceling_job),
        ("replayed", boolean()),
    ]);
    let mut completed_canceled = common.to_vec();
    completed_canceled.extend([
        ("controlState", json!({"const":"completed"})),
        ("cancellation", cancellation_completed.clone()),
        ("job", canceled_job.clone()),
        ("replayed", json!({"const":true})),
    ]);
    let mut completed_deferred = common.to_vec();
    completed_deferred.extend([
        ("controlState", json!({"const":"completed"})),
        ("cancellation", cancellation_completed),
        ("job", deferred_job),
        ("replayed", boolean()),
    ]);
    let mut indeterminate = common.to_vec();
    indeterminate.extend([
        ("controlState", json!({"const":"indeterminate"})),
        (
            "cancellation",
            strict_object(
                &[
                    ("id", agent_control_identifier()),
                    ("state", json!({"const":"indeterminate"})),
                    ("deliveryRoute", json!({"const":"runner_job"})),
                    ("requestedAt", agent_control_timestamp()),
                ],
                &["id", "state", "deliveryRoute", "requestedAt"],
    ...
    let required = [
        ...
        "cancellation",
        "job",
        "localCancellationAuthorized",
        "replayed",
    ];
    json!({
        "oneOf": [
            strict_object(&canceling, &required),
            strict_object(&completed_canceled, &required),
            strict_object(&completed_deferred, &required),
            strict_object(&indeterminate, &required),
        ]
    })
}

fn agent_model_identifier() -> Value {
    json!({
        "type":"string",
        "minLength":1,
        "maxLength":192,
        "pattern":"^[A-Za-z0-9][A-Za-z0-9._:@+-]*(?:/(?!\\.{1,2}(?:/|$))[A-Za-z0-9._:@+-]+)*$",
        "x-loomexCliIdentifierMaxBytes":192
    })
}

fn agent_control_identifier() -> Value {
    json!({
        "type":"string",
        "minLength":1,
        "maxLength":200,
        "pattern":"^[A-Za-z0-9][A-Za-z0-9._@+-]{0,199}$",
        "x-loomexControlIdentifierMaxBytes":200
    })
}

fn is_safe_agent_control_identifier(value: &str, max_bytes: usize) -> bool {
    !value.contains(['/', '\\', ':']) && is_safe_cli_identifier(value, max_bytes)
}

fn agent_control_timestamp() -> Value {
    json!({
        "type":"string",
        "minLength":20,
        "maxLength":64,
        "format":"date-time",
        "x-loomexRfc3339Timestamp":true
    })
}

fn is_safe_agent_control_timestamp(value: &str) -> bool {
    ...
    match bytes.get(timezone_index) {
        ...
        Some(b'+' | b'-') => {
            let offset = &bytes[timezone_index..];
            if offset.len() != 6 || offset.get(3) != Some(&b':') {
                return false;
            }
            let Some(offset_hour) = parse_ascii_number(&offset[1..3]) else {
                return false;
            };
            let Some(offset_minute) = parse_ascii_number(&offset[4..6]) else {
                return false;
            };
...
fn agent_public_error_schema() -> Value {
    strict_object(
        &[
            ("code", identifier()),
            (
...
fn agent_remediation_action_schema() -> Value {
    enum_string(&[
        "install_executor",
        "upgrade_executor",
        "refresh_executor_discovery",
        "authenticate",
...
fn agent_provider_schema() -> Value {
    enum_string(&["open_ai", "anthropic", "google"])
}

fn agent_executor_schema() -> Value {
    enum_string(&["codex_cli", "claude_cli", "agy_cli"])
}

fn agent_execution_state_schema() -> Value {
    enum_string(&[
        "queued",
        "probing",
        "blocked",
...
pub(crate) fn is_safe_cli_identifier(value: &str, max_bytes: usize) -> bool {
    ...
    if value.bytes().skip(1).any(|byte| {
...
fn agent_idempotency_key() -> Value {
    json!({
        "type":"string",
        "minLength":1,
        "maxLength":160,
        "pattern":"^[A-Za-z0-9][A-Za-z0-9._:/@+-]{0,159}$",
        "x-loomexIdempotencyKeyMaxBytes":160
    })
}
fn boolean() -> Value {
    json!({"type":"boolean"})
...
mod tests {
    ...
    fn output_fixture(name: &str) -> Value {
        ...
        match name {
            ...
            "loomex_agent_runtime_status" => json!({
            ...
            "loomex_agent_task_execute" | "loomex_agent_task_checkpoint" => json!({
                "requestId":"agent-1",
    ...
    #[test]
    fn agent_v2_inputs_are_minimal_strict_and_do_not_accept_execution_payloads() {
        let status = definition("loomex_agent_runtime_status").unwrap();
        assert_eq!(status.input_schema["properties"], json!({}));
        assert_eq!(status.input_schema["required"], json!([]));
        assert_eq!(status.input_schema["additionalProperties"], false);
        assert!(validate_arguments(&status.input_schema, &json!({})).is_ok());
        assert!(validate_arguments(&status.input_schema, &json!({"requestId":"agent-1"})).is_err());
        ...
        for (name, key_name) in [
            ...
            let properties = definition.input_schema["properties"]
                .as_object()
                .expect("properties");
            let allowed = [key_name, "requestId"].into_iter().collect::<HashSet<_>>();
            assert_eq!(
                properties
                    .keys()
                    .map(String::as_str)
            ...
            let mut valid = json!({"requestId":"agent-1"});
            valid[key_name] = json!("idem-agent-1");
            assert!(validate_arguments(&definition.input_schema, &valid).is_ok());
            assert!(
                validate_arguments(&definition.input_schema, &json!({"requestId":"agent-1"}))
                    .is_err()
            );
            for field in forbidden {
                let mut unsafe_input = valid.clone();
                unsafe_input[field] = json!("unsafe");
                assert!(
                    validate_arguments(&definition.input_schema, &unsafe_input).is_err(),
                    "{name} accepted {field}"
                );
            }
            let alias = if key_name == "idempotencyKey" {
                "operationIdempotencyKey"
            } else {
                "idempotencyKey"
            };
            let mut aliased = valid.clone();
            aliased.as_object_mut().unwrap().remove(key_name);
            aliased[alias] = json!("idem-agent-1");
            assert!(
                validate_arguments(&definition.input_schema, &aliased).is_err(),
            ...
            for invalid_key in [
                ...
                invalid[key_name] = json!(invalid_key);
                assert!(
                    validate_arguments(&definition.input_schema, &invalid).is_err(),
                    "{name} accepted invalid key {invalid_key:?}"
                );
            }
            let mut boundary = valid;
            boundary[key_name] = json!("a".repeat(160));
            assert!(validate_arguments(&definition.input_schema, &boundary).is_ok());
            boundary[key_name] = json!("a".repeat(161));
            assert!(validate_arguments(&definition.input_schema, &boundary).is_err());
        }
    }

    #[test]
    fn disabled_runtime_status_accepts_only_the_strict_v2_empty_snapshot() {
        let status = definition("loomex_agent_runtime_status").unwrap();
        let valid = envelope(
            status.name,
            json!({
        ...
        validate_output(&status.output_schema, &valid).unwrap();

        let mut null_runtimes = valid.clone();
        null_runtimes["data"]["runtimes"] = Value::Null;
        assert!(validate_output(&status.output_schema, &null_runtimes).is_err());

        let mut unknown_schema = valid;
        unknown_schema["data"]["schema"] = json!("loomex.agent-capabilities.v3");
        assert!(validate_output(&status.output_schema, &unknown_schema).is_err());
    }

    #[test]
    fn agent_v2_outputs_are_strict_safe_allowlists() {
        let forbidden = [
            "path",
            "executablePath",
            "token",
        ...
        for name in [
            ...
            let definition = definition(name).unwrap();
            let valid = envelope(name, output_fixture(name));
            validate_output(&definition.output_schema, &valid).unwrap();
            for field in forbidden {
                let mut unsafe_output = valid.clone();
                unsafe_output["data"][field] = json!("secret");
                assert!(
                    validate_output(&definition.output_schema, &unsafe_output).is_err(),
                    "{name} accepted data.{field}"
                );
            }
        }

        let status = definition("loomex_agent_runtime_status").unwrap();
        let mut unsafe_runtime = envelope(status.name, output_fixture(status.name));
        unsafe_runtime["data"]["runtimes"][0]["executablePath"] =
            json!("/Users/example/.local/bin/agy");
        assert!(validate_output(&status.output_schema, &unsafe_runtime).is_err());
        for invalid_model in [
            json!("/Users/example/private"),
            json!("model\n--help"),
            json!("-flag"),
            json!("vendor/../private"),
        ] {
            for field in ["modelKey", "providerModelId"] {
                let mut unsafe_model = envelope(status.name, output_fixture(status.name));
                unsafe_model["data"]["runtimes"][0]["models"][0][field] = invalid_model.clone();
                assert!(
                    validate_output(&status.output_schema, &unsafe_model).is_err(),
                    "runtime status accepted unsafe {field}"
                );
            }
        }

        let execute = definition("loomex_agent_task_execute").unwrap();
        let mut unresolved_auto = envelope(execute.name, output_fixture(execute.name));
        unresolved_auto["data"]
            .as_object_mut()
            .unwrap()
            .remove("modelKey");
        unresolved_auto["data"]
            .as_object_mut()
            .unwrap()
            .remove("providerModelId");
        validate_output(&execute.output_schema, &unresolved_auto).unwrap();
        let mut shortest_protocol_key = envelope(execute.name, output_fixture(execute.name));
        shortest_protocol_key["data"]["idempotencyKey"] = json!("a");
        validate_output(&execute.output_schema, &shortest_protocol_key).unwrap();
        shortest_protocol_key["data"]["idempotencyKey"] = json!("-invalid");
        assert!(validate_output(&execute.output_schema, &shortest_protocol_key).is_err());
        ...
        for valid in [
            ...
            assert!(is_safe_cli_identifier(valid, 192));
            let mut valid_segment = envelope(execute.name, output_fixture(execute.name));
            valid_segment["data"]["modelKey"] = json!(valid);
            valid_segment["data"]["providerModelId"] = json!(valid);
            validate_output(&execute.output_schema, &valid_segment)
                .unwrap_or_else(|error| panic!("rejected valid model identity {valid}: {error}"));
        }

        for missing_field in ["modelKey", "providerModelId"] {
            let mut one_sided = envelope(execute.name, output_fixture(execute.name));
            one_sided["data"]
                .as_object_mut()
                .unwrap()
                .remove(missing_field);
            assert!(
                validate_output(&execute.output_schema, &one_sided).is_err(),
                "accepted one-sided model identity without {missing_field}"
            );
        }
        ...
        for invalid in [
            ...
            let mut unsafe_model = envelope(execute.name, output_fixture(execute.name));
            unsafe_model["data"]["modelKey"] = invalid;
            assert!(validate_output(&execute.output_schema, &unsafe_model).is_err());
        }

        let mut unsafe_error = envelope(execute.name, output_fixture(execute.name));
        unsafe_error["data"]["error"] = json!({
            "code":"provider_not_authenticated",
            "message":"Authentication is required.",
            "retryable":false,
            "rawError":"Bearer secret",
        });
        assert!(validate_output(&execute.output_schema, &unsafe_error).is_err());

        let mut provider_not_eligible = envelope(execute.name, output_fixture(execute.name));
        provider_not_eligible["data"]["state"] = json!("blocked");
        provider_not_eligible["data"]["accepted"] = json!(false);
        provider_not_eligible["data"]["error"] = json!({
            "code":"provider_not_eligible",
            "message":"The current provider account is not eligible for this agent execution.",
            "retryable":false,
            "remediation":["verify_provider_access","contact_support"],
        });
        validate_output(&execute.output_schema, &provider_not_eligible).unwrap();

        let mut refresh_discovery = envelope(execute.name, output_fixture(execute.name));
        refresh_discovery["data"]["state"] = json!("blocked");
        refresh_discovery["data"]["accepted"] = json!(false);
        refresh_discovery["data"]["error"] = json!({
            "code":"provider_not_installed",
            "message":"The selected local agent provider is not installed.",
            "retryable":false,
            "remediation":["install_executor","refresh_executor_discovery"],
        });
        validate_output(&execute.output_schema, &refresh_discovery).unwrap();

        let mut upgrade_executor = envelope(execute.name, output_fixture(execute.name));
        upgrade_executor["data"]["state"] = json!("blocked");
        upgrade_executor["data"]["accepted"] = json!(false);
        upgrade_executor["data"]["error"] = json!({
            "code":"unsupported_capability",
            "message":"The selected local agent does not support a required capability.",
            "retryable":false,
            "remediation":["upgrade_executor","refresh_executor_discovery"],
        });
        validate_output(&execute.output_schema, &upgrade_executor).unwrap();

        let mut cancelled = envelope(execute.name, output_fixture(execute.name));
        cancelled["data"]["state"] = json!("cancelled");
        validate_output(&execute.output_schema, &cancelled).unwrap();
        cancelled["data"]["state"] = json!("dispatch_cancelled");
        assert!(
            validate_output(&execute.output_schema, &cancelled).is_err(),
            "agent receipt schema widened logical state to dispatch_cancelled"
        );

        let resume = definition("loomex_agent_task_resume").unwrap();
        let valid_resume = envelope(resume.name, output_fixture(resume.name));
        validate_output(&resume.output_schema, &valid_resume).unwrap();
        for mutation in [
            ("/data/controlState", json!("running")),
            ("/data/sequence", json!(0)),
            ("/data/predecessor/state", json!("completed")),
            ("/data/successor/jobStatus", json!("leased")),
            ("/data/successor/attemptNumber", json!(0)),
            ("/data/successor/mode", json!("spawn_unrelated_execution")),
        ] {
            let mut invalid = valid_resume.clone();
            *invalid.pointer_mut(mutation.0).unwrap() = mutation.1;
            assert!(
                validate_output(&resume.output_schema, &invalid).is_err(),
                "resume accepted invalid {}",
                mutation.0
            );
        }
        for pointer in [
            ...
            "/data/predecessor/processAttemptId",
            "/data/successor/processAttemptId",
            "/data/successor/jobId",
            ...
            for invalid_id in [
                ...
                json!("/Users/example/private"),
                json!("vendor/../private"),
                json!("Users/Alice/private"),
                json!("C:/Users/Alice/private"),
                json!(r"C:\Users\Alice\private"),
                json!("é"),
            ] {
                let mut invalid = valid_resume.clone();
                *invalid.pointer_mut(pointer).unwrap() = invalid_id;
                assert!(
                    validate_output(&resume.output_schema, &invalid).is_err(),
                    "resume accepted unsafe identity at {pointer}"
                );
            }
        }
        for invalid_timestamp in [
            ...
            let mut unsafe_timestamp = valid_resume.clone();
            unsafe_timestamp["data"]["authorizedAt"] = json!(invalid_timestamp);
            assert!(validate_output(&resume.output_schema, &unsafe_timestamp).is_err());
        }

        let cancel = definition("loomex_agent_task_cancel").unwrap();
        let valid_cancel = envelope(cancel.name, output_fixture(cancel.name));
        validate_output(&cancel.output_schema, &valid_cancel).unwrap();
        let mut zero_sequence_cancel = valid_cancel.clone();
        zero_sequence_cancel["data"]["sequence"] = json!(0);
        validate_output(&cancel.output_schema, &zero_sequence_cancel).unwrap();
        for mutation in [
            ("/data/localCancellationAuthorized", json!(true)),
            ("/data/cancellation/deliveryRoute", json!("direct")),
            ("/data/job/leaseVersion", json!(0)),
            ("/data/job/status", json!("canceled")),
            ("/data/cancellation/state", json!("completed")),
        ] {
            let mut invalid = valid_cancel.clone();
            *invalid.pointer_mut(mutation.0).unwrap() = mutation.1;
            assert!(
                validate_output(&cancel.output_schema, &invalid).is_err(),
                "cancel accepted invalid {}",
                mutation.0
            );
        }
        for pointer in [
            "/data/requestId",
            "/data/agentExecutionId",
            "/data/processAttemptId",
            "/data/cancellation/id",
            "/data/job/id",
        ] {
            for invalid_id in [
                json!("-flag"),
                ...
                *invalid.pointer_mut(pointer).unwrap() = invalid_id;
                assert!(
                    validate_output(&cancel.output_schema, &invalid).is_err(),
                    "cancel accepted unsafe identity at {pointer}"
                );
            }
        }
        for invalid_timestamp in [
            ...
            let mut unsafe_timestamp = valid_cancel.clone();
            unsafe_timestamp["data"]["cancellation"]["requestedAt"] = json!(invalid_timestamp);
            assert!(validate_output(&cancel.output_schema, &unsafe_timestamp).is_err());
        }
        let mut completed = valid_cancel;
        completed["data"]["controlState"] = json!("completed");
        completed["data"]["cancellation"]["state"] = json!("completed");
        completed["data"]["job"]["status"] = json!("canceled");
        completed["data"]["replayed"] = json!(true);
        validate_output(&cancel.output_schema, &completed).unwrap();
        completed["data"]["replayed"] = json!(false);
        assert!(validate_output(&cancel.output_schema, &completed).is_err());

        let mut indeterminate = envelope(cancel.name, output_fixture(cancel.name));
        indeterminate["data"]["controlState"] = json!("indeterminate");
        indeterminate["data"]["cancellation"]["state"] = json!("indeterminate");
        indeterminate["data"]["job"]["status"] = json!("canceled");
        indeterminate["data"]["replayed"] = json!(true);
        validate_output(&cancel.output_schema, &indeterminate).unwrap();
        indeterminate["data"]["replayed"] = json!(false);
        assert!(validate_output(&cancel.output_schema, &indeterminate).is_err());
        indeterminate["data"]["replayed"] = json!(true);
        indeterminate["data"]["controlState"] = json!("completed");
        assert!(validate_output(&cancel.output_schema, &indeterminate).is_err());

        for replayed in [false, true] {
            let mut deferred = envelope(cancel.name, output_fixture(cancel.name));
            deferred["data"]["controlState"] = json!("completed");
            deferred["data"]["cancellation"]["state"] = json!("completed");
            deferred["data"]["job"]["status"] = json!("deferred");
            deferred["data"]["replayed"] = json!(replayed);
            validate_output(&cancel.output_schema, &deferred).unwrap();
        }
    }

    #[test]
    fn unknown_output_contracts_fail_closed_instead_of_panicking() {
        let schema = output_schema("loomex_missing_agent_runtime_tool");
        let output = envelope("loomex_missing_agent_runtime_tool", json!({}));
